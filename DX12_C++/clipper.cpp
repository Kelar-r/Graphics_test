...
void ClipPolygonToAxis(clip_result* Input, clip_result* Output, clip_axis Axis)
{
	...
	for (u32 TriangleId = 0; TriangleId < Input->NumTriangles; ++TriangleId)
	{
		...
		{
			case Triangle_Is_Visible:
			{
				//Всі 3 точки трикутника в полі зору камери
			...
			case Smaler_Part_Triangle_Invisible:
			{
				//Одна точка трикутника поза камерою
				u32 CurrTriangleId = Output->NumTriangles;
				u32 CurrVertexId = 0;
			...
			case Most_Of_Triangle_Invisible:
			{
				//Дві точки трикутника поза камерою
				u32 CurrTriangleId = Output->NumTriangles++;
				u32 CurrVertexId = 0;
			...
			case Triangle_Is_Invisible:
			{
				//Всі точки трикутника за камерою
			} break;
